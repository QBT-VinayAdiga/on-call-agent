...
- `frontend` service:
  ...
  - Ports: `5173:5173`
  - Volumes: `./frontend/src:/app/frontend/src` (for HMR).
...
- Runs `uvicorn` in watch mode for development.

### 5.2. `Dockerfile.frontend`
- Installs `pnpm`.
- Installs frontend dependencies.
- Runs `vite` in dev mode.

### 5.3. `docker-compose.yml`
- Defines the services, networking, and volume mounts.
- Sets environment variables (e.g., `PORT=8000`).

## 6. Testing and Validation
- **Local Run:** Execute `podman-compose up` or `docker-compose up`.
